...
describe('integratoin test', () => {
  ...
  it('should registry customer get token and send taxpayer registry', async () => {
    ...
    res = await request(server)
      ...
      expect(res.status).toEqual(200)
      expect(res.body["next-end-point"]).toEqual("/api/v1/full-name")
  });

 
  it('should registry customer get token and send invalid endpoint order', async () => {
    ...
    res = await request(server)
      ...
      expect(res.body).toEqual({"error": "Invalid endpoint order"})
  });
