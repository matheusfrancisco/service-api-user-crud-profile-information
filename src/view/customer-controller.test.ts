...
describe('CustomerController', () => {
  ...
  let updateFullNameMock: (email: string, token: string, firstName: string, lastName: string) => Promise<void>;
  ...
  const expectedCustomer = [
    { id: 1,  email: 'matheusmachadoufsc@hotmail.com', password: 'ee65426e-b376-4221-931b-994913e17b73' },
  ];
  const req = {
    body: {
      email: customerEmail,
  ...
  class CustomerServiceMock extends CustomerService {
    constructor(
      ...
      updatedBirthday: (email: string, birthday: string) => Promise<any>
    ) {
      ...
      this.updatedBirthday = updatedBirthday;
    }
  }
  ...
  describe('POST create-customer', () => {
    it('Should receive name and email and password and delegate to application layer', async () => {
      ...
      expect(createCustomer).to.have.been.calledWith(
        customerEmail,
        userPass,
      );
    });
