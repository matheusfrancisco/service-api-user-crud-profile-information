...
const buildCustomerController = (customerService: CustomerService) => ({
  ...
  updateFullName: async(req: any, res: any) => {
    ...
    res.json({
      // ...customerUpdated,
      success: true,
      'next-end-point': customerUpdated?.nextEndpoint
    })
    res.status(201).end();
    return res
  },
  ...
  updatedBirthday: async(req: any, res: any) => {
    if (!req.body || !req.body.birthday ) {
      res.status(400).json(buildErrorMessage('Parameters missing'));
      return;
    }
    try {
      const customerUpdated = await customerService.updatedBirthday(
        req.body.email,
        req.body.birthday,
      );
      res.json({
  ...
  updatePhoneNumber: async(req: any, res: any) => {
    if (!req.body || !req.body.phoneNumber ) {
      res.status(400).json(buildErrorMessage('Parameters missing'));
      return;
    }
    try {
      const customerUpdated = await customerService.updatePhoneNumber(
        req.body.email,
  ...
  updateAddress: async(req: any, res: any) => {
    if (!req.body || !req.body.cep || !req.body.address || !req.body.city 
      || !req.body.number || !req.body.state) {
      res.status(400).json(buildErrorMessage('Parameters missing'));
      return;
    }
    try {
  ...
  addAmount: async(req: any, res: any) => {
    if (!req.body || !req.body.amount ) {
      res.status(400).json(buildErrorMessage('Parameters missing'));
      return;
    }
    try {
      const customerUpdated = await customerService.addAmount(
        req.body.email,
        req.body.amount,
      );
      res.json({
